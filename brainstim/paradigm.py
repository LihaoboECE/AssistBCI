...
from .utils import NeuroScanPort, NeuraclePort, Light_trigger, _check_array_like, Virtual_trigger
import threading
...
def paradigm_apply(
    ...
    port = None
    if get("device_state") == "connected":

        DEVICE_COMM_TOOL = {
            'NeuroScan': "NeuroScan",
            'BlueBCI': "Virtual_trigger",
            'Neuracle': "Neuracle",
        }
        DEVICE_COMM_PORT = {
            'NeuroScan': "COM8",
        ...
        device_type = DEVICE_COMM_TOOL[get("connect_device")]
        port_addr = DEVICE_COMM_PORT[get("connect_device")]
        ...
        if device_type == "NeuroScan":
            ...
            print("device connecting")
            port = Virtual_trigger(dict=_buffer)
        else:
            port = None
            print("Port cannot start")
            ...
            if port == None:
                ...
                for sf in range(VSObject.stim_frames):
                    ...
                    if time.time() - start >= 1 / fps:
                        print("Loose Frame, Frame time:", time.time() - start)
                    win.flip()
                    start = time.time()
            else:
                start = time.time()
                for sf in range(VSObject.stim_frames):
                    ...
                    if sf == 0:
                        VSObject.win.callOnFlip(port.setData, 1) #仅有一种tigger
                ...
                while not get("par_feedback"):
                    win.flip()
                send("par_feedback", False)
