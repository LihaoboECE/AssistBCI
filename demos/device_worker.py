...
from workers import BasicWorker, ControlWorker, EmptyWorker, OnlineTestWorker
import time
...
class Device(Process):
    ...
    def default(self):
        ...
        self.worker_list = {
            "BasicWorker": BasicWorker, 'ControlWorker': ControlWorker, "EmptyWorker": EmptyWorker, "OnlineTestWorker": OnlineTestWorker,
        }
        ...
        self.worker_default_parameters = {
            ...
            "OnlineTestWorker": self.save_hyper(timeout=1e-5, worker_name='feedback_worker')
        }

        self.marker_default_parameters = {
            'ControlWorker': self.save_hyper(interval=[0.14, 3.5], srate=1000, save_data=False, clear_after_use=False, events=[1,]),
            "EmptyWorker": self.save_hyper(interval=[0, 5], srate=1000, save_data=True, clear_after_use=False),
            "OnlineTestWorker": self.save_hyper(interval=[0, 5], srate=1000, save_data=True, clear_after_use=False)
        }
