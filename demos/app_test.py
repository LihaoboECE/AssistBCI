    ...
    def on_mouse_press(self, x, y, input, modifiers):
        if input == mouse.LEFT:
            if self.is_button_clicked(self.button_bigger, x, y):
                ...
                #         square.height -= 10
                self.square_len -= 10
                self.button_smaller.color = (0, 255, 0)
                for square in self.squares:
                    square.width = self.square_len
                    square.height = self.square_len
                ...
                freqs = np.arange(8, 8 + n_elements * 1 - 0.1, 1).tolist()  # 目前暂不支持自定义频率（最多支持8目标）
                phases = np.array([0 for i in range(n_elements)]).tolist()
...
class menu(APP):
    ...
    def control(self, dt):
        ...
        if not self.pause.is_set():
            try:
                if self.get('current_par') == '#menu' and not self.get('start_par'):
                    if self.hide.is_set():
                        self.hide.clear()
            except:
                pass
            ...
            elif self.get("sys_key") == 'f1':  # hide or not hide
                if self.hide.is_set():
                    self.hide.clear()
                else:
                    self.hide.set()
    ...
    def on_key_press(self, symbol, modifiers):
        ...
        elif symbol == pyglet.window.key.F1: #hide or not hide
            if self.hide.is_set():
                self.hide.clear()
            else:
                self.hide.set()
...
class Experiment(Process):
    ...
    def training(self, experiment_name, subject):
        ...
        from metabci.brainda.algorithms.decomposition import (
            FBTRCA, FBTDCA, FBSCCA, FBECCA, FBDSP, TRCA, TRCAR,TDCA,
    ...
    def start_experiment(self):
        try:
            ...
            self.parameters['freqs'] = np.arange(8, 8 + n_elements * 0.2 - 0.1, 0.2).tolist()  # 目前暂不支持自定义频率(最多支持40目标)
            self.parameters['phases'] = np.array([(i % 4) * 0.5 for i in range(n_elements)]).tolist()

            # self.parameters['freqs'] = np.arange(8, 8 + n_elements * 1 - 0.1, 1).tolist()  # 目前暂不支持自定义频率（最多支持8目标）
            # self.parameters['phases'] = np.array([0 for i in range(n_elements)]).tolist()

            self.send_hyper("marker", interval=[0.14, 5.14], srate=1000, save_data=True, info=self.parameters, clear_after_use=False, events=event,
            ...
            if not self.parameters['online']:
                ...
                self.send("reg_worker", "OnlineTestWorker")
